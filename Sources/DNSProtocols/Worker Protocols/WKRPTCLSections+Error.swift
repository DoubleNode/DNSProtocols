...
public extension DNSError {
    typealias Sections = WKRPTCLSectionsError
}
public enum WKRPTCLSectionsError: DNSError {
    // Common Errors
    ...
    public static let domain = "WKRSECTIONS"
    ...
    public var errorString: String {
        switch self {
            // Common Errors
        case .unknown:
            return String(format: NSLocalizedString("WKRSECTIONS-Unknown Error%@", comment: ""),
                          " (\(Self.domain):\(Self.Code.unknown.rawValue))")
        case .notImplemented:
            return String(format: NSLocalizedString("WKRSECTIONS-Not Implemented%@", comment: ""),
                          " (\(Self.domain):\(Self.Code.notImplemented.rawValue))")
        case .notFound(let field, let value, _):
            return String(format: NSLocalizedString("WKRSECTIONS-Not Found%@%@%@", comment: ""),
                          "\(field)", "\(value)",
        ...
        case .invalidParameters(let parameters, _):
            ...
            return String(format: NSLocalizedString("WKRSECTIONS-Invalid Parameters%@%@", comment: ""),
                          "\(parametersString)",
                          " (\(Self.domain):\(Self.Code.invalidParameters.rawValue))")
        case .lowerError(let error, _):
            return String(format: NSLocalizedString("WKRSECTIONS-Lower Error%@%@", comment: ""),
