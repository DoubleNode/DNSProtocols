...
public extension DNSError {
    typealias Promotions = WKRPTCLPromotionsError
}
public enum WKRPTCLPromotionsError: DNSError {
    // Common Errors
    ...
    public static let domain = "WKRPROMOTIONS"
    ...
    public var errorString: String {
        switch self {
            // Common Errors
        case .unknown:
            return String(format: NSLocalizedString("WKRPROMOTIONS-Unknown Error%@", comment: ""),
                          " (\(Self.domain):\(Self.Code.unknown.rawValue))")
        case .notImplemented:
            return String(format: NSLocalizedString("WKRPROMOTIONS-Not Implemented%@", comment: ""),
                          " (\(Self.domain):\(Self.Code.notImplemented.rawValue))")
        case .notFound(let field, let value, _):
            return String(format: NSLocalizedString("WKRPROMOTIONS-Not Found%@%@%@", comment: ""),
                          "\(field)", "\(value)",
        ...
        case .invalidParameters(let parameters, _):
            ...
            return String(format: NSLocalizedString("WKRPROMOTIONS-Invalid Parameters%@%@", comment: ""),
                          "\(parametersString)",
                          " (\(Self.domain):\(Self.Code.invalidParameters.rawValue))")
        case .lowerError(let error, _):
            return String(format: NSLocalizedString("WKRPROMOTIONS-Lower Error%@%@", comment: ""),
