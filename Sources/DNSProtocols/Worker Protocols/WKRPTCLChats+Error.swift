...
public extension DNSError {
    typealias Chats = WKRPTCLChatsError
}
public enum WKRPTCLChatsError: DNSError {
    // Common Errors
    ...
    public static let domain = "WKRCHATS"
    ...
    public var errorString: String {
        switch self {
            // Common Errors
        case .unknown:
            return String(format: NSLocalizedString("WKRCHATS-Unknown Error%@", comment: ""),
                          " (\(Self.domain):\(Self.Code.unknown.rawValue))")
        case .notImplemented:
            return String(format: NSLocalizedString("WKRCHATS-Not Implemented%@", comment: ""),
                          " (\(Self.domain):\(Self.Code.notImplemented.rawValue))")
        case .notFound(let field, let value, _):
            return String(format: NSLocalizedString("WKRCHATS-Not Found%@%@%@", comment: ""),
                          "\(field)", "\(value)",
        ...
        case .invalidParameters(let parameters, _):
            ...
            return String(format: NSLocalizedString("WKRCHATS-Invalid Parameters%@%@", comment: ""),
                          "\(parametersString)",
                          " (\(Self.domain):\(Self.Code.invalidParameters.rawValue))")
        case .lowerError(let error, _):
            return String(format: NSLocalizedString("WKRCHATS-Lower Error%@%@", comment: ""),
