...
public enum WKRPTCLBeaconDistError: DNSError {
    ...
    public var errorString: String {
        ...
        case .notFound(let field, let value, _):
            return String(format: NSLocalizedString("WKRBCONDIST-Not Found(%@=\"%@\")%@", comment: ""),
                          "\(field)", "\(value)",
        ...
        case .invalidParameters(let parameters, _):
            ...
            return String(format: NSLocalizedString("WKRBCONDIST-Invalid Parameters(%@)%@", comment: ""),
                          "\(parametersString)",
                          " (\(Self.domain):\(Self.Code.invalidParameters.rawValue))")
        case .lowerError(let error, _):
            return String(format: NSLocalizedString("WKRBCONDIST-Lower Error [%@]%@", comment: ""),
